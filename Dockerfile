...
RUN apt update && apt install -y sudo libluajit-5.1-dev luajit make cmake clang curl luarocks git libssl-dev autoconf libnice-dev libglib2.0-dev openjdk-8-jdk groovy2 ; apt clean
...
RUN cd /opt/lash/lib/ext/ctypes.sh && ./autogen.sh && ./configure && make && make install
